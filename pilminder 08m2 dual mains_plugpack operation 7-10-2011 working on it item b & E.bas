...
;declaring bug 7 not found - cant fix what aint broke!
;declaring development D done. Still needs hardware change to actually run off mains. Consider 1/2 wave rectifier?
...
'*7* fault not found - changeover from mains to internal timer, sometimes dumps sec and min timers - single pip might be a clue
...
'**B** needs testing Can have test/demo mode where everything happens quicky - test if bottle present on start or not *****
...
'**E** needs testing Allow user to specfify a day to NOT take meds if med count =1 (sat night for Tony S)
...
'**H** Remove pip at 24 hr rollover (2 of them) - havent heard them yet...
...
'**L** Swap outputs 1 and 2 then add tune during the skiped med istead of a boring beep
...
symbol 	Beeper = 1			'  output 1 on pin 6 (drives piezo directly)
symbol 	IR_TX_LED = 2		'  output 2 on pin 5 (drives IR led via 470ohm resistor) ** why ?? would leave tx led on all the time !! **
