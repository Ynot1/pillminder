...
'*12* done Sunlight/warmth affects the touch sensors, making pillmider think there is someone requesting an early dose.
'*13* done? - detection of foil up, or a nearly empty bottle up is tentative. Needs an aveage reading and a sudden change detection rather than threshold crossing
...
'**H** done Remove pip at 24 hr rollover (2 of them) 
...
'**M** done ? Experiment with checking IR level and looking for an abrubt change in level, rather than a crossing of a threshold.
...
'*P* batteries going flat is not graceful, it just stops flashing the leds. it does beep though...


'IR readings from Tan prototype
'Full Bottle - 7
'Full Foil 	 - 37
'Nearly MT Bottle - 100 (2 pills)
'Empty Bottle 	- 135
'Empty Foil - Varies 180 - 230
'Nothing in Pillminder - 245

' operation
'Bottle missing @ start = demo mode **B**
'Bottle present and static during med timer programming = 1 med per day, 
'bottle present and then touch sensor 2 touched X times = 'x' meds per 24 hr period
'speacial mode if touch sensor 2 touched 3x then touch sensor 1 touched. = skip this day and every 7th subsequent day **E**
...
symbol	TouchValueThresholdIncrement = 10 ;scope to iterate this......
symbol	BottleSensorThresholdIncrement = 20 ;scope to iterate this......
symbol	BottleSensorThresholdDecrement = 20 ;scope to iterate this......
...
symbol 	IR_Level_Average = b7

'b8 & b9 used as w4
symbol 	Med_Interval = w4
'b8 & b9 used as w4

symbol 	TouchValueAverage2 = b10
symbol	TouchValue = b11
...
symbol 	TouchValueThreshold1 = b20
symbol 	TouchValueThreshold2 = b21

'b22 & 23 used as w11 
symbol	PremptCheck = w11 ; needs to be a word, as constructed from TimeToNextMed
'b22 & 23 used as w11 

'symbol 	BottleSensorAverage = b24

'b24 & b25 unused

'b26 & b27 or w13 - unused
...
'touch1 only used to invoke the day skip next day function. Has a slice level check on this (TouchValueThreshold1)
	'Calc baseline for Touch Sensor
	touch TouchSensor1, TouchValueThreshold1
	for ScratchByte = 1 to 20
		touch TouchSensor1, TouchValue
...
'CalibrateTouch2:
'touch 2 has an average running on it (TouchValueAverage2) during normal operation around the premempt medication function
'has a slice level check (TouchValueThreshold2) for the med programinmg function

	'Calc baseline for Touch Sensor
	touch TouchSensor2, TouchValueAverage2
	for ScratchByte = 1 to 20
		touch TouchSensor2, TouchValue
	Let TouchValueAverage2 = TouchValue + TouchValueAverage2 / 2 
	next
	'set detect value based on baseline + add-value

	Let TouchValueThreshold2 = TouchValueAverage2 + TouchValueThresholdIncrement

;CalibrateBottleSensor:

;Calc baseline for Bottle Sensor

	high IR_TX_LED

	readadc IR_RX_led, TouchValue
	
	for ScratchByte = 1 to 20
		readadc IR_RX_led, TouchValue
		Let IR_Level_Average = TouchValue + IR_Level_Average / 2 
	next
	
	'set detect value based on baseline + add-value

	'Let IR_LevelThreshold = IR_Level_Average + IR_LevelThresholdIncrement
...
' touch sensor 1 X times in next 30 secs to allow user to nominate # times per day to take meds
	...
	if touchvalue > TouchValueThreshold2 then 'Bottle_Missing_Flag =1 or Bottle_Missing_Flag =1 or
		...
		;sound Beeper, (125,1)	
		for ScratchByte = 1 to MedsPerDay
			sound Beeper, (125,1)
			pause 600
		next ScratchByte
		ScratchByte = 0	;allows counter to be used again in Bottle_Check routine
...
;LoopBotBack: I think this is a hangover from old method of progamming by lifting the bottle
;	gosub Check_Bottle
...
'      	GoTo wait_Second_Flag1	'ensures a stable flash of the led, somewhat random otherwise - bug **1**
...
wait_Second_Flag:   	
	...
	If Second_Flag = 0 then 	
      ...
      Else
		...
		'** 12 ** pillminder should take an average of readings eah pass, and compare touchvale to that rather than a threshold
		' this should make it less sensative to warth scrwewing up the touch sensor
		
		touch touchSensor2, touchvalue ' Read touch sensor2
		
		Let ScratchByte = TouchValueAverage2 + TouchValueThresholdIncrement
		
		if touchvalue > ScratchByte then ' Finger is on sensor
			...
			;update TouchValueAverage2 using touchvalue as new value
			Let TouchValueAverage2 = TouchValue + TouchValueAverage2 / 2 
		endif'touchvalue > TouchValueAverage2 + TouchValueIncrement
...
'If medication is missing on the first pass thru Remind, alert tone is sounded immediately and until medication replaced

	gosub Toggle_Side_LED 'wink medication lights
	...
	If Bottle_Missing_Flag = 1 and RemindPass = 0 then 'If medication is missing on the first pass thru Remind, alert tone is sounded immediately and until medication replaced

		Gosub Toggle_Side_LED
		For ScratchByte = 0 to 5  
			sound Beeper, (125,1)
			pause 10
		Next ScratchByte
	Else
	...
	If Bottle_Missing_Flag = 1 and RemindPass = 1 then 'assume meds just taken
...
'Keeps an average of IR readings in IR_Level_Average and checks for a sudden change in this average of the value
'BottleSensorThresholdIncrement
'Sets Bottle_Missing flag if bottle not in holder
	

	high IR_TX_LED	'power up IR TX LED

	readadc IR_RX_led, TouchValue 'read IR sensor
			
;	If Bottle_Missing_Flag = 1 Then;Medication is currently missing - Expecting Medication to be put back, IR level will be dropping
		Let ScratchByte = IR_Level_Average + BottleSensorThresholdIncrement ;establish threshold using last known average IR level
		
		If touchvalue >= ScratchByte then ' Compare new IR level against threshold
			Let Bottle_Missing_Flag = 1 'Medication is still not in holder
			Let IR_Level_Average = TouchValue + IR_Level_Average / 2 ;update IR_LevelAverage using touchvalue as new value
		EndIf
		
		Let ScratchByte = IR_Level_Average - BottleSensorThresholdDecrement ;establish threshold using last known average IR level
		
		If touchvalue <= ScratchByte then ' Compare new IR level against threshold
			Let Bottle_Missing_Flag = 0 'Medication is back in holder
			Let IR_Level_Average = TouchValue + IR_Level_Average / 2 ;update IR_LevelAverage using touchvalue as new value
		EndIf
		
		
;	ElseIf Bottle_Missing_Flag = 0 Then 'Medication is currently present - Expecting Medication to be removed , IR level will be raising
		
		
		
	;EndIf

	low IR_TX_LED	'shut down IR TX LED
	

'Original theshold crossing code
'powers up ir led and reads adc connected to IR sensor
...
wait_Second_Flag_Demo:   	
	...
	If Second_Flag = 0 then 
      ...
      Else
            ...
            If ScratchByte > Demo_Interval then
             
            if Sched_Bottle_Check_Flag = 0 then 'and ScratchByte > Demo_Interval then
    			gosub Remind
