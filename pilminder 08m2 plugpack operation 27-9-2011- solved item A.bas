...
'*1* Reminder LED flashes inconsistently
...
'*5* Exec loop is being stopped in a 1:30 duty cycle by something

'IDEAS FOR DEVELOPMENT

'**A** User should be prompted to take 1st dose of meds as soon as programming complete
...
'**** Allow user to specfify a day to NOT take meds if med count =1 (sat night for Tony S)
...
'let MinCount = 715
'Let Late_Med_Interval = 30

'falls thru to execloop

'debug ' confirm med count 

'Preload timers so user prompted to take meds now programing is complete
...
' All subroutines must yield promptly, and NOT hold the processor. 
	'Interupt routine looks after counting mains pulses, and maintaining timing
    	...
    	gosub check_bottle		'checks user has replaced bottle 30 sec after reminder
     '	If MinFlag =1 Then
     '	 	debug
     '	EndIf
