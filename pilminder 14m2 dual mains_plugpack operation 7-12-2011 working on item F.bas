...
'a work in progress - need to test Prempt_SkipNextRemind_Flag logic
...
symbol	PreemptValue = 1439 '30 'mins
...
;the reminders will get later in the day as time goes on, but will always be the same time apart.
		if Prempt_SkipNextRemind_Flag =1 and Second_Flag = 1 and TimeToNextMed => Med_Interval then
			Prempt_SkipNextRemind_Flag =0
		endif
		
		if Preempt_Flag =1 or Second_Flag = 1 and TimeToNextMed => Med_Interval then
...
'here if medsperday = 1 or 2
		...
		if Preempt_Flag = 1 or Day_Roll_Over_Flag = 1 or MedsPerDay = 2 and Mincount = 720 and SecCount =0 then
			if Preempt_Flag =1 then
				Preempt_Flag =0
				Prempt_SkipNextRemind_Flag =1	
			endif
