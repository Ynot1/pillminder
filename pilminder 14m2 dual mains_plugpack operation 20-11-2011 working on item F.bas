...
symbol 	TouchValueThreshold = w11
...
CalibrateTouch:
	...
	touch16 TouchSensor, TouchValueThreshold
	for Demo_Counter = 1 to 20
		touch16 TouchSensor, TouchValue
		Let TouchValueThreshold = TouchValue + TouchValueThreshold / 2 
	next
	'set detect value based on baseline + add-value

	Let TouchValueThreshold = TouchValueThreshold + TouchValueThresholdIncrement
...
wait_Second_Flag:   	
	...
	If Second_Flag = 0 then 	
      ...
      Else
		...
		if touchvalue >TouchValueThreshold then
			for ConfirmCounter = 1 to MedsPerDay
			sound Beeper, (125,1)
			pause 600
			next ConfirmCounter
