...
'**O** its very quiet in the production prototype case...No holes to let sound out!
...
'touch sensor1 activated = 'x' meds per 24 hr period
...
symbol 	IR_Threshold = 20 'was 150		;  slice level for deciding if bottle present or not
...
symbol	TouchValueThresholdIncrement = 100 ;this value works sometimes with coil of hookup wite 1cm dia, and the thick wall prototype...
symbol	PreemptValue = 30 'mins. This allows user to request to take medication early by touching a sensor- by PreemptValue amount.
...
symbol	TouchSensor1 = C.4
symbol	TouchSensor2 = C.5
...
'B24 & b25 used as w12
symbol 	TouchValueThreshold2 = w13
'B26 & b27 used as w13
...
' touch sensor 1 X times in next 30 secs to allow user to nominate # times per day to take meds
	...
	if touchvalue >TouchValueThreshold1 then 'Bottle_Missing_Flag =1 or Bottle_Missing_Flag =1 or
      ...
      Else
		...
		if touchvalue >TouchValueThreshold1 then
			...
			if PremptCheck >= Med_Interval Then
				'here to reset timers and prompt for meds to be taken early
				if MedsPerDay >=3 then 
		  			TimeToNextMed = Med_Interval
				...
				if MedsPerDay = 2  then
					Mincount = 720; this will drag time forward by the difference beteen the current time and the preprogramed time
				endif
				
				if MedsPerDay = 1  then
					Mincount = 1440; this will drag time forward by the difference beteen the current time and the preprogramed time
				endif				
