...
symbol	Demo_interval = 10	;seconds
...
'=================================================================
Begin:
let 		Bottle_Check_Threshold = 30 ; 30 seconds normally, overwritten to 5 in demo mode
...
'here if medsperday = 1 or 2
		...
		if Second_Flag = 1 and Remind_Req_Flag = 1 then
			...
			Else ' here if skipday and its the normal time to take meds
				tune 1, 13,($49,$40,$44,$49,$4B,$44,$40,$4B,$50,$44,$40)
...
Check_Bottle_Average: 


'You can do pretty much what Jeremy suggests with a IIR (infinite impulse response) software filter which takes a running average of previous readings without the overhead of remembering them.

'assuming two word variables AVERAGE and TEMPERATURE:

'<code><pre><font size=2 face='Courier'>
'loopmain:
'AVERAGE = 15* AVERAGE
'Readtemp12 pin, TEMPERATURE
'AVERAGE=TEMPERATURE + AVERAGE / 16

'(do whatever you want with your new average here)

'goto loopmain

 

'This uses an IIR method, older readings drop off after a few itterations with integer arithmetic.

'This sort of running average is useful for all sorts of things like steering routines or any routine where you want readings to move slowly over time.


'powers up ir led and reads adc connected to IR sensor
'averages the reading across 15 counts using an IIR filter
'checks for an abrubt change of IR level (> IR_Change_Threshold) against the average
'should the value that is greater than IR_Change_Threshold be included in the average???
'prob not, I think the aveage should only be calculated with the botle in place...
...
' Sets Bottle_Missing flag if bottle not in holder
	...
	If  IR_Average_Level - IR_Level >  ;IR_Level tends to raise when bottle removed. 255 = saturated
	'ambient dark 60 - 5 
	'ambient sun 250 - 130
	
	IR_Average_Level = 15* IR_Average_Level
	
	IR_Average_Level= IR_level + IR_Average_Level / 16
	...
	'ambient dark
	'60 units bottle missing 
	'2-5 when in place .  may be scope to reduce tx current further
	
	'ambient full sun
	'bright sunshine a problem with dumb threshold crossing detection.
	'mt bottle in full sun reads 130, 
	'full bottle in full sun about 20 
	'bottle missing in full sun reads 250
...
wait_Second_Flag_Demo:   	
	...
	If Second_Flag = 0 then 
      ...
      Else
            ...
            If Demo_Counter > Demo_Interval then
             
            if Sched_Bottle_Check_Flag = 0 then 'and Demo_counter > Demo_Interval then
    			gosub Remind
    		'else
